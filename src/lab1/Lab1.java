...
class Order {
			...
			if (this.order.charAt(1) == 's') { //判断化简
				...
				while (p < this.orderLength) {
					...
					if (!(this.order.charAt(p) <= 'z' && this.order.charAt(p) >= 'a'))
						return 0;
					while (p < this.orderLength && this.order.charAt(p) <= 'z' 
							&& this.order.charAt(p) >= 'a') {
					...
					while (p < this.orderLength && this.order.charAt(p) <= '9' 
							&& this.order.charAt(p) >= '0') {
	 ...
	 * @throws 
	 */  
	public int find(final StringBuffer a, final int lena, final StringBuffer b, final int lenb) {
...
class Expression {
	 ...
	 * @throws 
	 */  
	void S(StringBuffer expression) 
	{
		StringBuffer temp2 = new StringBuffer();
		int bracelet = 0;
		int len2;

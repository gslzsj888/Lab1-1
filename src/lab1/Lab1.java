		...
		Expression myExpression=new Expression();//表达式对象
		    while(true)
		    {
				...
				else if(type==1)
		        {
		        	...
		        	myExpression.S(myExpression.expression);
...
class Order{
	...
	static StringBuilder simp=new StringBuilder("simplify");
	...
	int cnt_argue=0;//当前的变量数目
	
	StringBuffer derivation=new StringBuffer();
	int derivation_len=0;
    ...
    public void init(String preOrder){
        ...
        for(int i=0;i<preOrder.length();i++)
        {
            if(preOrder.charAt(i)>='A' && preOrder.charAt(i)<='Z')
                this.order.append((char)((int)preOrder.charAt(i)+(int)'a'-(int)'A'));
	    ...
	    if(this.order.charAt(0)=='!')
	    {
	        if(this.order.charAt(1)=='s')//判断化简
	        {
	            ...
	            while(p<this.orderLength)
	            {
	                ...
	                if(find(temp,temp.length(),expression.expression, expression.expression_len)!=1)
	                    return 0;
	        ...
	        {
	            if(this.order.charAt(1)!='d' || this.order.charAt(2)!='/' || this.order.charAt(3)!='d')
	                return 0;
	            ...
	            if(find(this.derivation,this.derivation_len,expression.expression,expression.getExpression_len())!=1)
	                return 0;
	            return 2;
	        }
	    }
	        ...
	        */
	        while(i<this.orderLength)
	        {
	            ...
	            else if(i<this.orderLength && this.order.charAt(i)>='a' && this.order.charAt(i)<='z')
	            {
	                ...
	                if(i<this.orderLength-1 && this.order.charAt(i)=='^' && !(this.order.charAt(i+1)<='9' && this.order.charAt(i+1)>='0'))
	                {
	                	return 0;//如果阶数不是整数，则非法
	                }
	                else if(i<this.orderLength && (this.order.charAt(i)=='+' || this.order.charAt(i)=='-' || this.order.charAt(i)=='*' || this.order.charAt(i)=='/' || this.order.charAt(i)=='^'))
	                {
	                    temp.append(this.order.charAt(i));
	                    i++;
	                }
	            ...
	            else if(this.order.charAt(i)>='0' && this.order.charAt(i)<='9')
	            {
	                ...
	                if(i<this.orderLength-1 && this.order.charAt(i)=='^' && !(this.order.charAt(i+1)<='9' && this.order.charAt(i+1)>='0'))
	                	return 0;
	                else if(i<this.orderLength && ((this.order.charAt(i)<='z' && this.order.charAt(i)>='a') || this.order.charAt(i)=='('))
	                	temp.append('*');//补'*'号
	                else if(i<this.orderLength && (this.order.charAt(i)=='+' || this.order.charAt(i)=='-' || this.order.charAt(i)=='*' || this.order.charAt(i)=='/' || this.order.charAt(i)=='^'))
	                {
	                    temp.append(this.order.charAt(i));
	        ...
	        if(this.order.charAt(this.orderLength-1)=='+' || this.order.charAt(this.orderLength-1)=='-' || this.order.charAt(this.orderLength-1)=='*' || this.order.charAt(this.orderLength-1)=='/' || this.order.charAt(this.orderLength-1)=='^')
	        if(bracelet!=0)
	            return 0;
	...
	public int find(StringBuffer a,int lena,StringBuffer b,int lenb)
	{
	    for(int i=0;i<=lenb-lena;i++)
	    {
	        ...
	        if(flag==1 && (i-1<0 || !(b.charAt(i-1)<='z' && b.charAt(i-1)>='a')) && ((i+lena)>=lenb || !(b.charAt(i+lena)<='z' && b.charAt(i+lena)>='a')))
	            return 1;
	    }
	    return 0;
	}
	...
	public int find2(StringBuffer a,int lena)
	{
	    for(int i=0;i<cnt_argue;i++)
	    {
	        ...
	        {
	            if(arguement_name[i].subSequence(0, arguement_name[i].length()).equals(a.subSequence(0, a.length())))//equal(Object)
	            {
	            	return i;
	            }//替换值不能为0
	        }
...
class Expression{
	...
	public void substitution(Order order)
	{
	    ...
	    for(int i=0;i<this.expression_len;i++)
	    {
	        if(this.expression.charAt(i)>='a' && this.expression.charAt(i)<='z')
	        {
	            ...
	            while(i<this.expression_len && this.expression.charAt(i)>='a' && this.expression.charAt(i)<='z')
	            {
	...
	public void adjust(Order order)
	{
	    ...
	    for(int i=0;i<this.expression_len;i++)
	    {
	        if(this.expression.charAt(i)=='*')
	        {
	            ...
	            len2=0;   
	            while(p2<this.expression_len && this.expression.charAt(p2)>='0' && this.expression.charAt(p2)<='9')
	            {
	            ...
	            if((p1>=0 && (this.expression.charAt(p1)=='^')) || (p2<this.expression_len && (this.expression.charAt(p2)=='^')))
	                continue;
	            char sign=this.expression.charAt(i);
	            
	            //this.expression.replace(p1+1, p1+len1, temp2.substring(0, len1-1));
	    ...
	    for(int i=0;i<this.expression_len;i++)
	    {
	        if(this.expression.charAt(i)=='+' || this.expression.charAt(i)=='-')
	        {
	            ...
	            if((p1>=0 && (this.expression.charAt(p1)=='*' || this.expression.charAt(p1)=='/' || this.expression.charAt(p1)=='^'))
	             || (p2<this.expression_len && (this.expression.charAt(p2)=='*' || this.expression.charAt(p2)=='/' || this.expression.charAt(p2)=='^')))
	                continue;
	...
	void S(StringBuffer expression)//递归，len2
	{
	    StringBuffer temp2=new StringBuffer();
	    int bracelet=0;
	    int len2;
	    for(int i=0;i<expression.length();i++)//?
	    {
	        ...
	        {   
	            ...
	            while(bracelet!=0)
	            {
	                if(expression.charAt(i)==')')
	                    bracelet--;
	                //if(expression.charAt(i)=='(')bracelet++;
	    ...
	    */
	    ...
	    N1:	for(int i=0;i<expression_len;i++)
	    {
	        if(expression.charAt(i)=='*' || expression.charAt(i)=='/')
	        {
	            ...
	            if(expression.charAt(i)=='*')
	            	...
	            	sign=2;
	            if(!(expression.charAt(i-1)>='0' && expression.charAt(i-1)<='9' && expression.charAt(i+1)>='0' && expression.charAt(i+1)<='9'))
	            {
	            	if(temp2.length()-1<len2)
	    				temp2.append("$");
	                temp2.setCharAt(len2,expression.charAt(i));
	            ...
	            if(((p1>=0 && expression.charAt(p1)=='^') || (p2<expression_len && expression.charAt(p2)=='^')))
	            {
	            	if(temp2.length()-1<len2)
	    				temp2.append("$");
	            	temp2.setCharAt(len2,expression.charAt(i));
	    ...
	    N2:for(int i=0;i<expression_len;i++)
	    {
	        if(expression.charAt(i)=='+' || expression.charAt(i)=='-')
	        {
	            ...
	            if(((expression.charAt(i-1)>='a' && expression.charAt(i-1)<='z')|| expression.charAt(i-1)==')')|| ((expression.charAt(i+1)>='a' && expression.charAt(i+1)<='z')|| expression.charAt(i+1)=='('))
	            {///之前进不来为什么？
	            ...
	            if((p1>=0 && (expression.charAt(p1)=='*' || expression.charAt(p1)=='^' || expression.charAt(p1)=='/')) || (p2<expression_len && (expression.charAt(p2)=='*' || expression.charAt(p2)=='^' || expression.charAt(p2)=='/')))
	            {
	            	if(temp2.length()-1<len2)
	    				temp2.append("$");
	            	temp2.setCharAt(len2,expression.charAt(i));

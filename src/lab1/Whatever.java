...
public class Whatever {
	...
	class Object{
		int typ,num,e,sign;
		//0 for destroy,1 for numbers,2 for arguments,3 for +,4 for -,5 for *,6 for /;
		String argName;
	}
			...
			if(ord.typ==1){
				...
				for(int i=0;i<obj.size();i++){
					...
					if(ind!=-1){///
						newMem=obj.get(i);
						newMem.typ=1;
						newMem.num=ord.value.elementAt(ind);//不论哪个都越界，估计是vector没读进去
						obj.set(i, newMem);
					}
					if(obj.get(i).e==0){
						newMem=new Object();
